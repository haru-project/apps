...
def main():
    ...
    with lock.open('w') as handle:
        ...
        try:
            ...
            run('disk-reserve',['./ws','python3','-c',"import shutil,json; s=shutil.disk_usage('.'); reserve=5*1024**3; print(json.dumps({'available_bytes':s.free,'worker_reserve_bytes':reserve})); assert s.free>reserve, 'Insufficient space for the native worker disk reserve'"])
